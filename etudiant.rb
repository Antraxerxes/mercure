...
class Etudiant
    ...
    def ajoutVoeu( voeu )
        @voeux << Voeu.new( voeu[3].value, voeu[1].value, voeu[2].value, voeu[4].value, voeu[5].value )
    end
    ...
    def checkAdmission( listcritere )
        voeux.each do |voeu|
            ...
            if critere != false
                ...
                #Check note TOEFL
                if @noteTOEFL[0] != 0 && ( @noteTOEFL[0] < critere.critereTOEFL[0] || @noteTOEFL[1] < critere.critereTOEFL[1] || @noteTOEFL[2] < critere.critereTOEFL[2] || @noteTOEFL[3] < critere.critereTOEFL[3] || @noteTOEFL[4] < critere.critereTOEFL[4])
                    voeu.statut = false
                    voeu.failedTOEFL = true 
                end
                #check note IELTS
                if @noteIELTS[0] != 0 && (@noteIELTS[0] < critere.critereIELTS[0] || @noteIELTS[1] < critere.critereIELTS[1] || @noteIELTS[2] < critere.critereIELTS[2] || @noteIELTS[3] < critere.critereIELTS[3]|| @noteIELTS[4] < critere.critereIELTS[4])
                    voeu.statut = false
